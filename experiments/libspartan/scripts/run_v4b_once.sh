...
workload="${1:?expected W1, W2, W3, or W4}"
experiment="${2:?expected E0, E1, E2, E3, E4, B0, or B1}"
log_size="${3:?expected log size}"
cap_mib="${4:-uncapped}"
repetition="${5:-1}"

case "$workload" in W1|W2|W3|W4) ;; *) exit 2 ;; esac
case "$experiment" in E0|E1|E2|E3|E4|B0|B1) ;; *) exit 2 ;; esac

runner="$PWD/target/release/phase_v2_pbmo"
out_dir="$PWD/../credential_workloads/results/runs"
session="v4b-${workload}-${experiment}-${cap_mib}-${repetition}-$$"
state_root="/tmp/thinwallet-$session"
prefix="$out_dir/${workload}_${experiment}_${cap_mib}_r${repetition}"
mkdir -p "$out_dir" "$state_root/v3a" "$state_root/v3b"

cleanup() {
  case "$state_root" in
    /tmp/thinwallet-v4b-*) find "$state_root" -type f -delete 2>/dev/null || true
...
python3 - "$prefix" "$workload" "$experiment" "$provider" "$log_size" "$cap_mib" "$repetition" "$status" "$start_ns" "$end_ns" "$verify_status" <<'PY'
import hashlib, json, re, sys
from pathlib import Path

prefix, workload, experiment, provider, logn, cap, rep, status, start, end, verify_status = sys.argv[1:]
