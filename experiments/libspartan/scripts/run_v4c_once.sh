...
out_dir="$PWD/../credential_workloads/results/v4c/runs"
safe_workload="${workload//-/_}"
session="v4c-${safe_workload}-${experiment}-${cap_mib}-${repetition}-$$"
...
cleanup() {
  case "$state_root" in
    /tmp/thinwallet-v4c-*) find "$state_root" -type f -delete 2>/dev/null || true
