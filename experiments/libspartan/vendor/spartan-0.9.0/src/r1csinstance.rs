...
impl R1CSDecommitment {
  pub(crate) fn export_remote_external_state(&self) -> std::io::Result<Vec<u8>> {
    self.dense.export_remote_external_state()
  }

  pub(crate) fn import_remote_external_state(&mut self, bytes: &[u8]) -> std::io::Result<()> {
    self.dense.import_remote_external_state(bytes)
  }
}
