...
const WITNESS_DENSE_MLE: AllocationClass = AllocationClass {
  source_file: "src/r1csproof.rs",
  function: "R1CSProof::prove:poly_vars",
...
const PHASE_ONE_TABLES: AllocationClass = AllocationClass {
  source_file: "src/r1csproof.rs",
  function: "R1CSProof::prove:phase_one_tables",
...
const PHASE_TWO_TABLES: AllocationClass = AllocationClass {
  source_file: "src/r1csproof.rs",
  function: "R1CSProof::prove:phase_two_tables",
  component: "Sumcheck folded tables",
  element_type: "Scalar",
  privacy: "mixed",
  replayable: true,
...
const POLY_EVAL_STATE: AllocationClass = AllocationClass {
  source_file: "src/r1csproof.rs",
  function: "R1CSProof::prove:PolyEvalProof",
...
impl R1CSProof {
  ...
  pub fn prove(
    ...
    let trace_root = random_tape.root_label();
    ...
    let (poly_vars, comm_vars, blinds_vars) = {
      let _memory_scope = memory_trace::scope(&WITNESS_DENSE_MLE);
    ...
    let z = {
      let _memory_scope = memory_trace::scope(&PHASE_ONE_TABLES);
    ...
    let (mut poly_tau, mut poly_Az, mut poly_Bz, mut poly_Cz) = {
      ...
      let (poly_Az, poly_Bz, poly_Cz) = inst.multiply_vec(inst.get_num_cons(), z.len(), &z);
    ...
    let (sc_proof_phase1, rx, _claims_phase1, blind_claim_postsc1) = {
      #[cfg(feature = "thinwallet-experiment")]
      let _phase = thinwallet_instrumentation::PhaseGuard::begin("sumcheck");
      R1CSProof::prove_phase_one(
        num_rounds_x,
        &mut poly_tau,
        &mut poly_Az,
        &mut poly_Bz,
    ...
    #[cfg(feature = "thinwallet-experiment")]
    thinwallet_instrumentation::record_trace_event(
      "commit_claims_phase1",
      &["poly_Az", "poly_Bz", "poly_Cz", "rx"],
      &["comm_claims1"],
      Some(trace_root),
      &["comm_claims1"],
      false,
    ...
    let evals_ABC = {
      let _memory_scope = memory_trace::scope(&PHASE_TWO_TABLES);
      // compute the initial evaluation table for R(\tau, x)
    ...
    let _memory_scope = memory_trace::scope(&POLY_EVAL_STATE);
    let eval_vars_at_ry = poly_vars.evaluate(&ry[1..]);
    let blind_eval = random_tape.random_scalar(b"blind_eval");
    let (proof_eval_vars_at_ry, comm_vars_at_ry) = {
      #[cfg(feature = "thinwallet-experiment")]
      let _phase = thinwallet_instrumentation::PhaseGuard::begin("opening");
      PolyEvalProof::prove(
        &poly_vars,
        Some(&blinds_vars),
        &ry[1..],
    ...
    thinwallet_instrumentation::record_trace_event(
      "poly_eval_proof",
      &["poly_vars", "blinds_vars", "ry", "G", "H"],
      &["open_state", "proof_eval_vars_at_ry"],
      Some(trace_root),
