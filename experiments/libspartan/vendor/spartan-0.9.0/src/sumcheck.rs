...
static ACTIVE_SUMCHECK_STREAMING_NS: AtomicU64 = AtomicU64::new(0);

pub(crate) fn active_sumcheck_streaming_ns() -> u64 {
  ACTIVE_SUMCHECK_STREAMING_NS.load(Ordering::Relaxed)
}

pub(crate) fn reset_active_sumcheck_streaming_ns() {
  ACTIVE_SUMCHECK_STREAMING_NS.store(0, Ordering::Relaxed);
}
...
impl SumcheckInstanceProof {
  ...
  pub(crate) fn prove_cubic_batched_streaming<F>(
    claim: &Scalar,
    num_rounds: usize,
    poly_pairs_par: &mut [(StreamingPolynomial, StreamingPolynomial)],
    poly_C_par: &mut DensePolynomial,
    poly_triples_seq: &mut [(
      StreamingPolynomial,
      StreamingPolynomial,
      StreamingPolynomial,
    )],
    coeffs: &[Scalar],
    object_prefix: &str,
    store: &mut super::multi_state_store::MultiObjectFileBackedStateStore,
    comb_func: F,
    transcript: &mut Transcript,
  ) -> io::Result<(
    Self,
    ...
    F: Fn(&Scalar, &Scalar, &Scalar) -> Scalar,
  {
    ...
    let _phase = thinwallet_instrumentation::PhaseGuard::begin("sumcheck_external_folding");
    #[cfg(feature = "thinwallet-experiment")]
    thinwallet_instrumentation::increment_counter("external_fold_rounds", num_rounds as u64);
    use super::multi_state_store::ProverStateStore;
    let streaming_started = Instant::now();

    let mut e = *claim;
    let mut r = Vec::new();
    let mut cubic_polys = Vec::new();
    let chunk_width = StreamingPolynomial::preferred_chunk_scalars(store);

    for round in 0..num_rounds {
      let mut evals = Vec::with_capacity(poly_pairs_par.len() + poly_triples_seq.len());
      for (poly_A, poly_B) in poly_pairs_par.iter() {
        let half = poly_A.scalar_count / 2;
        if poly_B.scalar_count != poly_A.scalar_count || poly_C_par.len() != poly_A.scalar_count {
          return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "parallel streaming polynomial length mismatch",
          ));
        }
        let mut eval_point_0 = Scalar::zero();
        let mut eval_point_2 = Scalar::zero();
        let mut eval_point_3 = Scalar::zero();
        let mut a_reader = poly_A.open_reader(store)?;
        let mut b_reader = poly_B.open_reader(store)?;
        for start in (0..half).step_by(chunk_width) {
          let count = chunk_width.min(half - start);
          let a_low = poly_A.read_segment_from(store, &mut a_reader, start, count)?;
          let a_high = poly_A.read_segment_from(store, &mut a_reader, half + start, count)?;
          let b_low = poly_B.read_segment_from(store, &mut b_reader, start, count)?;
          let b_high = poly_B.read_segment_from(store, &mut b_reader, half + start, count)?;
          for offset in 0..count {
            let index = start + offset;
            eval_point_0 += comb_func(&a_low[offset], &b_low[offset], &poly_C_par[index]);
            let a2 = a_high[offset] + a_high[offset] - a_low[offset];
            let b2 = b_high[offset] + b_high[offset] - b_low[offset];
            let c2 = poly_C_par[half + index] + poly_C_par[half + index] - poly_C_par[index];
            eval_point_2 += comb_func(&a2, &b2, &c2);
            let a3 = a2 + a_high[offset] - a_low[offset];
            let b3 = b2 + b_high[offset] - b_low[offset];
            let c3 = c2 + poly_C_par[half + index] - poly_C_par[index];
            eval_point_3 += comb_func(&a3, &b3, &c3);
          }
        }
        evals.push((eval_point_0, eval_point_2, eval_point_3));
      }

      for (poly_A, poly_B, poly_C) in poly_triples_seq.iter() {
        let half = poly_A.scalar_count / 2;
        if poly_B.scalar_count != poly_A.scalar_count || poly_C.scalar_count != poly_A.scalar_count
        {
          return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "sequential streaming polynomial length mismatch",
          ));
        }
        let mut eval_point_0 = Scalar::zero();
        let mut eval_point_2 = Scalar::zero();
        ...
        let mut a_reader = poly_A.open_reader(store)?;
        let mut b_reader = poly_B.open_reader(store)?;
        let mut c_reader = poly_C.open_reader(store)?;
        for start in (0..half).step_by(chunk_width) {
          let count = chunk_width.min(half - start);
          let a_low = poly_A.read_segment_from(store, &mut a_reader, start, count)?;
          let a_high = poly_A.read_segment_from(store, &mut a_reader, half + start, count)?;
          let b_low = poly_B.read_segment_from(store, &mut b_reader, start, count)?;
          let b_high = poly_B.read_segment_from(store, &mut b_reader, half + start, count)?;
          let c_low = poly_C.read_segment_from(store, &mut c_reader, start, count)?;
          let c_high = poly_C.read_segment_from(store, &mut c_reader, half + start, count)?;
          for offset in 0..count {
            eval_point_0 += comb_func(&a_low[offset], &b_low[offset], &c_low[offset]);
            let a2 = a_high[offset] + a_high[offset] - a_low[offset];
            let b2 = b_high[offset] + b_high[offset] - b_low[offset];
            let c2 = c_high[offset] + c_high[offset] - c_low[offset];
            eval_point_2 += comb_func(&a2, &b2, &c2);
            let a3 = a2 + a_high[offset] - a_low[offset];
            let b3 = b2 + b_high[offset] - b_low[offset];
            let c3 = c2 + c_high[offset] - c_low[offset];
            eval_point_3 += comb_func(&a3, &b3, &c3);
          }
        }
        evals.push((eval_point_0, eval_point_2, eval_point_3));
      ...
      let poly = UniPoly::from_evals(&round_evals);
      poly.append_to_transcript(b"poly", transcript);
      let r_j = transcript.challenge_scalar(b"challenge_nextround");
      r.push(r_j);

      for (index, (poly_A, poly_B)) in poly_pairs_par.iter_mut().enumerate() {
        poly_A.fold_top(
          store,
          format!("{object_prefix}.r{round}.par{index}.a"),
          &r_j,
          round,
        )?;
        poly_B.fold_top(
          store,
          format!("{object_prefix}.r{round}.par{index}.b"),
          &r_j,
          round,
        )?;
      }
      poly_C_par.bound_poly_var_top(&r_j);
      for (index, (poly_A, poly_B, poly_C)) in poly_triples_seq.iter_mut().enumerate() {
        poly_A.fold_top(
          store,
          format!("{object_prefix}.r{round}.seq{index}.a"),
          &r_j,
          round,
        )?;
        poly_B.fold_top(
          store,
          format!("{object_prefix}.r{round}.seq{index}.b"),
          &r_j,
          round,
        )?;
        poly_C.fold_top(
          store,
          format!("{object_prefix}.r{round}.seq{index}.c"),
          &r_j,
          round,
        )?;
      }
    ...
    let mut claims_A_par = Vec::with_capacity(poly_pairs_par.len());
    let mut claims_B_par = Vec::with_capacity(poly_pairs_par.len());
    for (poly_A, poly_B) in poly_pairs_par.iter() {
      claims_A_par.push(poly_A.read_scalar(store)?);
      claims_B_par.push(poly_B.read_scalar(store)?);
      store.delete_object(&poly_A.object_id)?;
      store.delete_object(&poly_B.object_id)?;
    }
    let claims_prod = (claims_A_par, claims_B_par, poly_C_par[0]);
    let mut claims_A_seq = Vec::with_capacity(poly_triples_seq.len());
    let mut claims_B_seq = Vec::with_capacity(poly_triples_seq.len());
    let mut claims_C_seq = Vec::with_capacity(poly_triples_seq.len());
    for (poly_A, poly_B, poly_C) in poly_triples_seq.iter() {
      claims_A_seq.push(poly_A.read_scalar(store)?);
      claims_B_seq.push(poly_B.read_scalar(store)?);
      claims_C_seq.push(poly_C.read_scalar(store)?);
      store.delete_object(&poly_A.object_id)?;
      store.delete_object(&poly_B.object_id)?;
      store.delete_object(&poly_C.object_id)?;
    }

    ACTIVE_SUMCHECK_STREAMING_NS.fetch_add(
      streaming_started.elapsed().as_nanos() as u64,
      Ordering::Relaxed,
    );
    Ok((
      SumcheckInstanceProof::new(cubic_polys),
      r,
      claims_prod,
      (claims_A_seq, claims_B_seq, claims_C_seq),
    ))
  }
}
