...
METRICS_PATH = os.path.join(RESULTS, "holder_authorization_metrics.json")
...
def parse_r1cs_info(path, notes):
    ...
    for key, pattern in patterns.items():
        ...
        if match:
            ...
            notes.append(f"Could not parse {key} from build/holder_authorization/r1cs_info.txt.")
    return fields
...
def artifact_sizes():
    paths = {
        "r1cs_file_size_bytes": os.path.join(BUILD, "holder_authorization_main.r1cs"),
        "wasm_file_size_bytes": os.path.join(BUILD, "holder_authorization_main_js", "holder_authorization_main.wasm"),
        "proving_key_size_bytes": os.path.join(BUILD, "holder_authorization_final.zkey"),
...
def constraint_breakdown(nonlinear_constraints, linear_constraints):
    return {
        ...
        "source_components": [
            ...
            "BabyJubJub additions/doublings",
            "enabled equality checks forcing verifier success",
        ],
        "notes": [
            "Circom reports aggregate nonlinear/linear counts for the compiled verifier.",
            "Fine-grained EdDSA gadget subcomponent counts are not directly reported by this toolchain and are not inferred.",
        ],
    }
...
if __name__ == "__main__":
    try:
        ...
        print(f"collect_holder_authorization_metrics.py: {exc}", file=sys.stderr)
        sys.exit(1)
