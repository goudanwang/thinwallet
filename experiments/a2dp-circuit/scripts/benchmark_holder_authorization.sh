...
ROOT = os.environ["HOLDER_AUTH_ROOT"]
BUILD = os.path.join(ROOT, "build", "holder_authorization")
RESULTS = os.path.join(ROOT, "results")
SNARKJS = os.path.join(ROOT, "node_modules", ".bin", "snarkjs")
RUNS = 5

R1CS = os.path.join(BUILD, "holder_authorization_main.r1cs")
WASM = os.path.join(BUILD, "holder_authorization_main_js", "holder_authorization_main.wasm")
WITNESS_JS = os.path.join(BUILD, "holder_authorization_main_js", "generate_witness.js")
ZKEY = os.path.join(BUILD, "holder_authorization_final.zkey")
VKEY = os.path.join(BUILD, "verification_key.json")
VALID_INPUT = os.path.join(ROOT, "inputs", "holder_authorization_valid.json")
...
def run_command(command):
    ...
    wrapped = command
    if TIME_BIN:
        fd, time_file = tempfile.mkstemp(prefix="holder-auth-time-v-", text=True)
        os.close(fd)
...
required = [R1CS, WASM, WITNESS_JS, ZKEY, VKEY, VALID_WITNESS]
missing = [rel(path) for path in required if not os.path.exists(path)]
if missing:
    raise SystemExit("Missing build artifacts. Run scripts/build_holder_authorization.sh first: " + ", ".join(missing))
...
raw = {
    "experiment": "holder_authorization",
...
raw_path = os.path.join(RESULTS, "holder_authorization_raw.json")
