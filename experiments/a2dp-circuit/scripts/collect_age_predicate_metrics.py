...
BUILD = os.path.join(ROOT, "build", "age_predicate")
RESULTS = os.path.join(ROOT, "results")
RAW_PATH = os.path.join(RESULTS, "age_predicate_raw.json")
METRICS_PATH = os.path.join(RESULTS, "age_predicate_metrics.json")
...
INCLUDED_COMPONENTS = [
    ...
    "birth_day_index <= cutoff_day_index comparison",
    "Groth16 proof and verification for the age predicate circuit",
]
...
def parse_r1cs_info(path, notes):
    ...
    for key, pattern in patterns.items():
        ...
        if match:
            ...
            notes.append(f"Could not parse {key} from build/age_predicate/r1cs_info.txt.")
    return fields


def parse_nonlinear_constraints(path, notes):
    if not os.path.exists(path):
        notes.append("Missing compile log; nonlinear_constraints is null.")
        return None, None
    with open(path, "r", encoding="utf-8", errors="replace") as handle:
        text = strip_ansi(handle.read())
    match = re.search(r"non-linear constraints:\s*(\d+)", text)
    if not match:
        notes.append("Could not parse nonlinear constraints from compile.log.")
        return None, None
    return int(match.group(1)), "circom compile.log direct output"


def parse_linear_constraints(path):
    if not os.path.exists(path):
        return None
    with open(path, "r", encoding="utf-8", errors="replace") as handle:
        text = strip_ansi(handle.read())
    match = re.search(r"^linear constraints:\s*(\d+)", text, re.MULTILINE)
    return int(match.group(1)) if match else None


def parse_invalid_rejection(path, notes):
    result = {
    ...
    if not os.path.exists(path):
        notes.append("Missing invalid rejection log.")
        return result
    ...
    with open(path, "r", encoding="utf-8", errors="replace") as handle:
        for line in handle:
            ...
            if not sep:
                continue
            if key == "invalid_witness_exit_status":
                result["witness_exit_status"] = int(value)
            elif key == "invalid_check_exit_status":
    ...
    if result["expected_failure"] and (witness_failed or check_failed):
        ...
        notes.append("Invalid input rejection was not confirmed by non-zero witness or check status.")
    return result
...
def artifact_sizes():
    paths = {
        "r1cs_file_size_bytes": os.path.join(BUILD, "age_predicate_main.r1cs"),
        "wasm_file_size_bytes": os.path.join(BUILD, "age_predicate_main_js", "age_predicate_main.wasm"),
        "proving_key_size_bytes": os.path.join(BUILD, "age_predicate_final.zkey"),
...
def constraint_breakdown(linear_constraints):
    return {
        "birth_day_index_Num2Bits_32": {
            "constraints": 32,
            "type": "nonlinear",
        },
        "cutoff_day_index_Num2Bits_32": {
            "constraints": 32,
            "type": "nonlinear",
        },
        "LessEqThan_32_internal_Num2Bits_33": {
            "constraints": 33,
            "type": "nonlinear",
        },
        "force_comparison_output_to_one": {
            "constraints": linear_constraints,
            "type": "linear",
        },
        "notes": [
            "The circuit intentionally uses two independent Num2Bits(32) range checks plus the LessEqThan(32) internal bit decomposition.",
            "The nonlinear constraint count is read from Circom compile output, not inferred from total constraints.",
        ],
    }
...
def main():
    ...
    nonlinear_constraints, nonlinear_source = parse_nonlinear_constraints(compile_log, notes)
    linear_constraints = parse_linear_constraints(compile_log)
    ...
    metrics = {
        ...
        "constraint_breakdown": constraint_breakdown(linear_constraints),
        "negative_test": parse_invalid_rejection(os.path.join(BUILD, "invalid_rejection.log"), notes),
...
if __name__ == "__main__":
    try:
        ...
        print(f"collect_age_predicate_metrics.py: {exc}", file=sys.stderr)
        sys.exit(1)
