...
ROOT = os.environ["AGE_ROOT"]
BUILD = os.path.join(ROOT, "build", "age_predicate")
RESULTS = os.path.join(ROOT, "results")
SNARKJS = os.path.join(ROOT, "node_modules", ".bin", "snarkjs")
RUNS = 5

R1CS = os.path.join(BUILD, "age_predicate_main.r1cs")
WASM = os.path.join(BUILD, "age_predicate_main_js", "age_predicate_main.wasm")
WITNESS_JS = os.path.join(BUILD, "age_predicate_main_js", "generate_witness.js")
ZKEY = os.path.join(BUILD, "age_predicate_final.zkey")
VKEY = os.path.join(BUILD, "verification_key.json")
VALID_INPUT = os.path.join(ROOT, "inputs", "age_valid.json")
...
def run_command(command):
    ...
    wrapped = command
    if TIME_BIN:
        fd, time_file = tempfile.mkstemp(prefix="age-time-v-", text=True)
        os.close(fd)
...
if missing:
    raise SystemExit("Missing build artifacts. Run scripts/build_age_predicate.sh first: " + ", ".join(missing))
