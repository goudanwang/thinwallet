...
export REQUEST_BINDING_ROOT="$ROOT"
...
ROOT = os.environ["REQUEST_BINDING_ROOT"]
BUILD = os.path.join(ROOT, "build", "request_binding")
RESULTS = os.path.join(ROOT, "results")
SNARKJS = os.path.join(ROOT, "node_modules", ".bin", "snarkjs")
RUNS = 5

R1CS = os.path.join(BUILD, "request_binding_main.r1cs")
WASM = os.path.join(BUILD, "request_binding_main_js", "request_binding_main.wasm")
WITNESS_JS = os.path.join(BUILD, "request_binding_main_js", "generate_witness.js")
ZKEY = os.path.join(BUILD, "request_binding_final.zkey")
VKEY = os.path.join(BUILD, "verification_key.json")
VALID_INPUT = os.path.join(ROOT, "inputs", "request_binding_valid.json")
...
def run_command(command):
    ...
    wrapped = command
    if TIME_BIN:
        fd, time_file = tempfile.mkstemp(prefix="request-binding-time-v-", text=True)
        os.close(fd)
...
if missing:
    raise SystemExit("Missing build artifacts. Run scripts/build_request_binding.sh first: " + ", ".join(missing))
