...
export ONLINE_PRESENTATION_ROOT="$ROOT"
...
ROOT = os.environ["ONLINE_PRESENTATION_ROOT"]
BUILD = os.path.join(ROOT, "build", "online_presentation")
RESULTS = os.path.join(ROOT, "results")
SNARKJS = os.path.join(ROOT, "node_modules", ".bin", "snarkjs")
RUNS = 5

R1CS = os.path.join(BUILD, "online_presentation.r1cs")
WASM = os.path.join(BUILD, "online_presentation_js", "online_presentation.wasm")
WITNESS_JS = os.path.join(BUILD, "online_presentation_js", "generate_witness.js")
ZKEY = os.path.join(BUILD, "online_presentation_final.zkey")
VKEY = os.path.join(BUILD, "verification_key.json")
VALID_INPUT = os.path.join(ROOT, "inputs", "online_presentation_valid.json")
...
def run_command(command):
    ...
    wrapped = command
    if TIME_BIN:
        fd, time_file = tempfile.mkstemp(prefix="online-presentation-time-v-", text=True)
        os.close(fd)
...
if missing:
    raise SystemExit("Missing build artifacts. Run scripts/build_online_presentation.sh first: " + ", ".join(missing))
