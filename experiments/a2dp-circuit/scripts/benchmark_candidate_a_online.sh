...
export CANDIDATE_A_ONLINE_ROOT="$ROOT"
...
ROOT = os.environ["CANDIDATE_A_ONLINE_ROOT"]
BUILD = os.path.join(ROOT, "build", "candidate_a_online")
RESULTS = os.path.join(ROOT, "results")
SNARKJS = os.path.join(ROOT, "node_modules", ".bin", "snarkjs")
RUNS = 5

R1CS = os.path.join(BUILD, "candidate_a_online_presentation.r1cs")
WASM = os.path.join(BUILD, "candidate_a_online_presentation_js", "candidate_a_online_presentation.wasm")
WITNESS_JS = os.path.join(BUILD, "candidate_a_online_presentation_js", "generate_witness.js")
ZKEY = os.path.join(BUILD, "candidate_a_online_final.zkey")
VKEY = os.path.join(BUILD, "verification_key.json")
VALID_INPUT = os.path.join(ROOT, "inputs", "candidate_a_valid.json")
...
SIGN_SCRIPT = os.path.join(BUILD, "holder_sign_request.js")
VERIFY_SCRIPT = os.path.join(BUILD, "external_signature_verify.js")
...
def run_command(command):
    ...
    wrapped = command
    if TIME_BIN:
        fd, time_file = tempfile.mkstemp(prefix="candidate-a-online-time-v-", text=True)
        os.close(fd)
...
required = [R1CS, WASM, WITNESS_JS, ZKEY, VKEY, VALID_WITNESS, SIGN_SCRIPT, VERIFY_SCRIPT]
missing = [rel(path) for path in required if not os.path.exists(path)]
if missing:
    raise SystemExit("Missing build artifacts. Run scripts/build_candidate_a_online.sh first: " + ", ".join(missing))

stages = {
    "holder_signing": ["node", SIGN_SCRIPT],
    "external_signature_verification": ["node", VERIFY_SCRIPT, "valid"],
    "witness_generation": ["node", WITNESS_JS, WASM, VALID_INPUT, BENCH_WITNESS],
    "prove": [SNARKJS, "groth16", "prove", ZKEY, VALID_WITNESS, BENCH_PROOF, BENCH_PUBLIC],
    "verify": [
        SNARKJS,
        "groth16",
        "verify",
...
raw = {
    ...
    "tool_versions": {
        ...
        "circomlibjs": version(["node", "-e", "const fs=require('fs'); console.log(JSON.parse(fs.readFileSync('node_modules/circomlibjs/package.json','utf8')).version)"]),
    },
    "stages": {},
}
