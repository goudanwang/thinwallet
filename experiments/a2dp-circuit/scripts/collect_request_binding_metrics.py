...
RAW_PATH = os.path.join(RESULTS, "request_binding_raw.json")
METRICS_PATH = os.path.join(RESULTS, "request_binding_metrics.json")
...
INCLUDED_COMPONENTS = [
    ...
    "public expected_request_digest equality check",
    "Groth16 proof and verification for the request binding circuit",
]
...
def parse_r1cs_info(path, notes):
    ...
    for key, pattern in patterns.items():
        ...
        if match:
            ...
            notes.append(f"Could not parse {key} from build/request_binding/r1cs_info.txt.")
    return fields
...
def artifact_sizes():
    paths = {
        "r1cs_file_size_bytes": os.path.join(BUILD, "request_binding_main.r1cs"),
        "wasm_file_size_bytes": os.path.join(BUILD, "request_binding_main_js", "request_binding_main.wasm"),
        "proving_key_size_bytes": os.path.join(BUILD, "request_binding_final.zkey"),
...
def constraint_breakdown(nonlinear_constraints, linear_constraints):
    return {
        "Poseidon_6_inputs": {
            ...
            "partial_rounds": 63,
            "s_box_slots": 119,
            "active_s_boxes_after_constant_folding": 118,
            "constant_folded_s_boxes": 1,
            "constraints_per_s_box": 3,
        ...
        "notes": [
            "Poseidon(6) uses t = 7. The 8 full rounds apply 56 S-boxes and the 63 partial rounds apply 63 S-boxes.",
            "The first S-box slot receives a constant initial-state branch and is constant-folded by Circom in this compiled circuit.",
            "The remaining 118 active x^5 S-boxes compile to three multiplication constraints each, for 354 nonlinear constraints.",
            "The equality to expected_request_digest is absorbed without a separately reported linear constraint in this compiled R1CS.",
        ],
    }
...
def main():
    ...
    metrics = {
        ...
        "negative_tests": {
            "invalid_nonce": parse_rejection("invalid", notes),
            "invalid_domain": parse_rejection("invalid_domain", notes),
        },
...
if __name__ == "__main__":
    try:
        ...
        print(f"collect_request_binding_metrics.py: {exc}", file=sys.stderr)
        sys.exit(1)
