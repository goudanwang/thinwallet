...
ROOT = os.environ["DISCLOSURE_ROOT"]
BUILD = os.path.join(ROOT, "build", "disclosure_control")
RESULTS = os.path.join(ROOT, "results")
SNARKJS = os.path.join(ROOT, "node_modules", ".bin", "snarkjs")
RUNS = 5

R1CS = os.path.join(BUILD, "disclosure_control_main.r1cs")
WASM = os.path.join(BUILD, "disclosure_control_main_js", "disclosure_control_main.wasm")
WITNESS_JS = os.path.join(BUILD, "disclosure_control_main_js", "generate_witness.js")
ZKEY = os.path.join(BUILD, "disclosure_control_final.zkey")
VKEY = os.path.join(BUILD, "verification_key.json")
VALID_INPUT = os.path.join(ROOT, "inputs", "disclosure_valid.json")
...
def run_command(command):
    ...
    wrapped = command
    if TIME_BIN:
        fd, time_file = tempfile.mkstemp(prefix="disclosure-time-v-", text=True)
        os.close(fd)
...
if missing:
    raise SystemExit("Missing build artifacts. Run scripts/build_disclosure_control.sh first: " + ", ".join(missing))
